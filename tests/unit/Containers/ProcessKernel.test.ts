...
describe("Test process kernel", async () => {
    ...
    test("KernelProcess called messageShutdownListen", async () => {
        const logger = container.get(ContainerName.Logger);
        const processKernel = container.get(ContainerName.ProcessKernel);
    ...
    test("KernelProcess uncaughtException without logger", async () => {
        const processKernel = container.get(ContainerName.ProcessKernel);

        vi.spyOn(processKernel.container, "getOptional").mockReturnValue(undefined);
    ...
    test("KernelProcess messageShutdownListen without logger", async () => {
        const processKernel = container.get(ContainerName.ProcessKernel);
